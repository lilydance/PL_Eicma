...
export default function DataGridDemo() {
  ...
  const [tarjetasDudosas, setTarjetasDudosas] = React.useState([]);
  ...
  const getTarjetasDudosas = React.useCallback(() => {
    setLoaded(false);
    fetch(
      `http://${process.env.REACT_APP_REST_API_URL}/transporte/mant-anual/${value.format("YYYY")}`
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        ...
        setTarjetasDudosas(filteredData);
      })
      .catch((err) => console.log(err))
      .finally(() => setLoaded(true));
  }, [setLoaded, value, setTarjetasDudosas]);
        ...
        <Box sx={{ margin: 3, marginTop: 5 }}>
          <center>
            <Typography variant="h7">
              <strong>Anexo 1: Tarjetas Dudosas.</strong>
            </Typography>
          </center>
          <br></br>
          <DatePicker
